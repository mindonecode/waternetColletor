...
public class QuartzCronScheduler {
	 ...
	 */
	public void schedule(Class<? extends Job> jobClass, String cronExpr, String jobNm, String cronTriggerNm,
			String group) throws SchedulerException {

		// 파라미터 체크
		if ("".equals(cronExpr) == true || "".equals(jobNm) == true || "".equals(cronTriggerNm) == true
				|| "".equals(group) == true) {
		...
		// 특정 시간에 작동하도록 스케쥴 설정
		CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity(cronTriggerNm, group)
	...
	public void shutDown() throws SchedulerException {
		...
		if (scheduler == null) {
			logger.info("shutDown cron scheduler not started..");
			return;
		}
		...
		if (isStarted == true) {
			scheduler.shutdown();
			logger.info("shutDown cron scheduler shutdown..");
		}
