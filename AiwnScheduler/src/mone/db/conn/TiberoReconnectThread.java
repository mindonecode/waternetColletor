...
public class TiberoReconnectThread implements Runnable {
	...
	public void run() {
		...
		try {
			...
			logger.info("Exception tiberoDbConnVal 1st");
		}
		...
		if (DbManager.tiberoDbConnVal < 0) {
			...
			try {
				...
				logger.info("Exception connTibero ");
			}
			...
			try {
				...
				logger.info("Exception tiberoDbConnVal 2nd");
			}
			if (DbManager.tiberoDbConnVal < 0) {
				logger.info("(OK)db connected  ::: TiberoDB ");
			}
		} else {
			logger.info("(OK)db connected  ::: TiberoDB ");
		}
